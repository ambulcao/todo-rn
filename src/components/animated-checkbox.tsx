...
const AnimatedCheckbox = (props: Props) => {
  ...
  const animatedBoxProps = useAnimatedProps(() => ({
    stroke: `rgb(${
      (1 - progress.value) * parseInt(boxOutlineColor.slice(1, 3), 16) +
      progress.value * parseInt(highlightColor.slice(1, 3), 16)
    }, ${
      (1 - progress.value) * parseInt(boxOutlineColor.slice(3, 5), 16) +
      progress.value * parseInt(highlightColor.slice(3, 5), 16)
    }, ${
      (1 - progress.value) * parseInt(boxOutlineColor.slice(5), 16) +
      progress.value * parseInt(highlightColor.slice(5), 16)
    })`,
    fill: `rgb(${
      (1 - progress.value) * 0 + progress.value * parseInt(highlightColor.slice(1, 3), 16)
    }, ${
      (1 - progress.value) * 0 + progress.value * parseInt(highlightColor.slice(3, 5), 16)
    }, ${
      (1 - progress.value) * 0 + progress.value * parseInt(highlightColor.slice(5), 16)
    })`,
  }));
